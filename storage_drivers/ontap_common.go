...
func PopulateConfigurationDefaults(config *OntapStorageDriverConfig) error {
	...
	if config.SplitClone == "" {
		config.SplitClone = DefaultSplitClone
	}
...
func CreateOntapClone(name, source, snapshot, splitClone string, api *ontap.Driver) error {
